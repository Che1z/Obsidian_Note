...
WHERE file.path != this.file.path AND (title AND category) AND startDate AND !finishDate
SORT startDate DESC
```
...
FROM "80_Personal/Reading"
WHERE file.path != this.file.path AND (title AND category) AND finishDate AND date(finishDate).year = date("today").year
